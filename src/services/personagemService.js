...
function validateUniverse(universeId) {
  ...
  if (settings.allowOnlyDefaultUniverse && universeId !== settings.defaultUniverse) {
    return { ok: false, message: `Por enquanto só existe o *Universo ${settings.defaultUniverse}*.` };
  }
...
function getOrCreatePlayer(message) {
  ...
  const displayName = message._data?.notifyName || message._data?.pushName || null;

  db.prepare(`
    INSERT INTO players (whatsapp_id, phone, display_name)
    VALUES (?, ?, ?)
    ON CONFLICT(whatsapp_id) DO UPDATE SET
      phone = excluded.phone,
      display_name = COALESCE(excluded.display_name, players.display_name),
      updated_at = CURRENT_TIMESTAMP
  `).run(whatsappId, phone, displayName);
...
function registerCharacter(message, universeId, characterName, rescueCode = null) {
  ...
  const isSupreme = normalizeText(player.cargo) === 'supremo';
  ...
  if (!isSupreme) {
    ...
    const supremeOccupied = getClaimByCharacter(universeId, character.id, 'supremo');
    if (supremeOccupied) {
      return { ok: false, message: `*${character.name}* já está sendo usado por alguém com Cargo Supremo.` };
    }
  }
  ...
  return {
    ...
    message: [
      ...
      isSupreme ? '👑 Cargo Supremo: esse personagem não ocupou vaga comum do universo.' : null,
      '',
...
function generateRescueCode(message, universeId, characterName) {
  ...
  const createdBy = message.author || message.from;
  let code;
  let inserted = false;
